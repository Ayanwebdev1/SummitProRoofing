    ...
    <>
      <nav className="bg-white/70 backdrop-blur sticky top-0 z-50 border-gray-200 shadow h-[90px] flex items-center justify-between px-10">
        ...
        <Link href="/"><div className="flex items-center h-full gap-2 cursor-pointer">
          ...
          <h1 className="text-[25px] font-bold text-[#004099]">SUMMITPRO<span className="text-[#e85b30]">ROOFING</span></h1>
        </div></Link>
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <Link href="/getaquote"><button className="bg-orange-500 cursor-pointer text-white font-semibold px-5 py-2 rounded-md hover:bg-orange-600 transition">
            GET A QUOTE
          </button></Link>
        </div>
      </nav>
          ...
          <ul className="flex flex-col gap-4 px-5 py-2 text-gray-700 font-semibold">
            {navLinks.map(([name, link]) => (
              <Link key={name} href={link} onClick={() => setIsOpen(false)}>
                <li
                  className={`cursor-pointer transition px-3 py-2 rounded-md ${pathname === link ? "bg-orange-600 text-white" : "hover:bg-green-100"
                    }`}
                >
                  {name}
                </li>
            ...
            <button className="bg-orange-500 cursor-pointer text-white font-semibold px-5 py-2 rounded-md hover:bg-orange-600 transition">
              GET A QUOTE
            </button>
          </ul>
        </div>
